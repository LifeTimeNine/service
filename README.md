...
> `notify`方法返回的数据是可以直接返回微信服务器的消息；回调方法中可以接收两个参数，第一个是微信返回的数据，第二个是签名验证结果；如果回调方法返回值为false则直接给微信返回失败的消息
...
参考 [微信支付开发文档](https://pay.weixin.qq.com/wiki/doc/apiv3/wxpay/pages/index.shtml)
